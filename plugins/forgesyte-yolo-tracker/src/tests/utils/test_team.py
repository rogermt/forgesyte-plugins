...
class TestTeamClassifier:
    ...
    def test_initialization(self) -> None:
        ...
        with patch('forgesyte_yolo_tracker.utils.team.SiglipVisionModel') as mock_model, \
    ...
    def test_fit_requires_crops(self) -> None:
        ...
        with patch('forgesyte_yolo_tracker.utils.team.SiglipVisionModel'), \
             patch('forgesyte_yolo_tracker.utils.team.AutoProcessor'):
            ...
            classifier.extract_features = MagicMock(
                return_value=np.random.rand(1, 512)
            )
            classifier.reducer.fit_transform = MagicMock(
                return_value=np.random.rand(1, 3)
            )
    ...
    def test_predict_before_fit(self) -> None:
        ...
        with patch('forgesyte_yolo_tracker.utils.team.SiglipVisionModel'), \
            ...
            classifier.reducer.transform = MagicMock(
                return_value=np.random.rand(1, 3)
            )
            classifier.cluster_model.predict = MagicMock(
